...
namespace stroganov_m_multiplication_double_crs_matrix_seq {
  ...
  void MakeCRS(std::vector<unsigned int> &r_i, std::vector<unsigned int> &col, std::vector<double> &val,
    ...
    // Инициализация
    r_i = std::vector<unsigned int>(m + 1, 0); // m + 1 для индексов строк
    col.clear();
    val.clear();
    ...
    for (unsigned int i = 0; i < m; i++) {
      for (unsigned int j = 0; j < n; j++) {
        if (src[(i * n) + j] != 0) {
          val.push_back(src[(i * n) + j]);  // Добавляем ненулевой элемент в значания
          col.push_back(j);                  // Добавляем индекс столбца
          r_i[i + 1]++;                      // Увеличиваем количество ненулевых элементов в строке
        }
      }
    }

    // Пересчитываем индексы для строк
